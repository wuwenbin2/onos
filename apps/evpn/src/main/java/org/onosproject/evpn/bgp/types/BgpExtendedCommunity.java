...
public class BgpExtendedCommunity implements BgpValueType {
    ...
    private static final Logger log = LoggerFactory
            .getLogger(BgpExtendedCommunity.class);
     ...
     */
    public static BgpExtendedCommunity read(ChannelBuffer cb)
        ...
        // if fourth bit is set, length is read as short otherwise as byte , len
        // includes type, length and value
        int len = validation.isShort()
                                       ? validation.getLength()
                                               + Constants.TYPE_AND_LEN_AS_SHORT
                                       : validation.getLength()
                                               + Constants.TYPE_AND_LEN_AS_BYTE;
        ChannelBuffer data = tempCb.readBytes(len);
        if (validation.getFirstBit() && !validation.getSecondBit()
                && validation.getThirdBit()) {
        ...
        if (tempBuf.readableBytes() > 0) {
            ...
            ChannelBuffer bgpExtComBuf = tempBuf
                    .readBytes(validation.getLength());
            ...
            while (bgpExtComBuf.readableBytes() > 0) {
                ...
                case Constants.BGP_ENCAP:
                    ...
                    break;
                default:
                    log.debug("Other type Not Supported:" + type);
                    break;
                }
    ...
    public String toString() {
        return MoreObjects.toStringHelper(getClass()).omitNullValues()
                .add("fsActionTlv", fsActionTlv).toString();
    }
